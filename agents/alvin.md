...
- The Essay Topic.
- The numbered Outline structure (each outline has an index, type, title, and description).
...
3. Each source must directly support the content of a specific outline paragraph — choose the outline whose topic is most closely matched by the source.
4. Distribute sources across the outlines. Do not assign all sources to the same outline. Body paragraphs should each receive at least one supporting source.
5. Your output MUST be strictly in JSON format. Do not include any markdown formatting blocks (like ```json), conversational text, or explanations. ONLY output the raw JSON array.
...
- "outline_index" (number): The 1-based index of the outline this source best supports (e.g. 1 for the first outline, 2 for the second, etc.).
