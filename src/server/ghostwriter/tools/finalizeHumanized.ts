...
// Call this after `humanize_essay` succeeds. If StealthGPT collapsed the
// essay into one block, optionally call `split_paragraphs` first.
//
...
export const finalizeExportHumanizedTool: Tool<FinalizeHumanizedArgs, FinalizeHumanizedResult> = {
  ...
  description:
    "Package the humanized essay into an export snapshot. Reads ctx.humanizedContent (set by humanize_essay, optionally refined by split_paragraphs). Emits humanizedExportDoc so the client can show the humanized editor and download card. Call this after humanize_essay, with or without split_paragraphs.",
  parameters: {
    type: "object",
  ...
  async execute(_args, { run }) {
    ...
    if (!ctx.humanizedContent?.trim()) {
      throw new Error(
        "finalize_export_humanized: humanizedContent is missing — run humanize_essay first.",
      );
    }
